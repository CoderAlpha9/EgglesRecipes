...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # --- State Management ---
        # UPGRADE: Added "prices" array to track the Bollinger Band window
        trader_state = {
            "EMERALDS_FEAT": {"spread":[], "imb":[], "mdiv":[], "vol":[]},
            "TOMATOES_FEAT": {"spread":[], "imb":[], "mdiv":[], "vol":[], "prices":[]},
            "OFI_STATE": {},  
            "VPIN_STATE": {}  
        }
        ...
        window_len = 10 
        bb_window_len = 15 # Slightly longer window for reliable Bollinger Standard Deviation
        vpin_window_len = 15 
        ...
        for product in state.order_depths:
            ...
            if len(vpin_hist) > vpin_window_len:
                vpin_hist.pop(0)
            ...
            # Bolinger Band Price Array Tracking
            if "prices" in feats:
                feats["prices"].append(mid_price)
                if len(feats["prices"]) > bb_window_len: feats["prices"].pop(0)
            ...
            # ==========================================
            # EXECUTION: TOMATOES (Bollinger Engine)
            # ==========================================
            if product == "TOMATOES":
                # --- BOLLINGER BAND CALCULATION ---
                bb_std = 1.0 # Default safe volatility
                bb_upper = mid_price + 1.0
                bb_lower = mid_price - 2.0
                
                if len(feats["prices"]) >= window_len:
                    bb_mean = sum(feats["prices"]) / len(feats["prices"])
                    bb_var = sum((x - bb_mean)**2 for x in feats["prices"]) / len(feats["prices"])
                    bb_std = math.sqrt(bb_var)
                    # 2 Standard Deviations creates the momentum bands
                ...
                fair_price = mid_price + alpha

                # --- 1. STAGNATION FREQUENCY REDUCTION ---
                stagnation_penalty = 0.0
                # If std is incredibly low (< 1.2), the market is flatlining. Widen the maker spread.
                if bb_std < 1.2:
                    stagnation_penalty = 1.0 

                # --- DYNAMIC TAKER SWEEPING BASE ---
                buy_edge_required = 0.5 + (current_pos / limit) * 2.0
                sell_edge_required = 0.5 - (current_pos / limit) * 2.0

                # --- 2 & 3. MOMENTUM REVERSION & LIQUIDITY EXPLOITATION ---
                # If price drops below the Lower Bollinger Band, it's an extreme oversold dip. 
                # Be aggressive: demand LESS edge to buy.
                if mid_price <= bb_lower:
                    buy_edge_required -= 1.5 
                
                # If price bursts above the Upper Bollinger Band, exploit the fake liquidity pump.
                # Be aggressive: demand LESS edge to sell.
                if mid_price >= bb_upper:
                    sell_edge_required -= 1.5 

                # Sweep the book with the new Bollinger-adjusted edges
                ...
                # Notice we add the stagnation_penalty to widen the spread during flatlines
