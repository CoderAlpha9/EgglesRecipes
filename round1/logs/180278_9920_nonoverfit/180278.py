...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                orders = self.trade_pepper(product, order_depth, position, trader_state)
    ...
    # ================================================================
    def trade_pepper(self, product, od: OrderDepth, position, ts):
        orders: List[Order] = []
        ...
        prev_best_ask = ts.get("prev_best_ask")

        # Condition 1:
        # First timestamp → always buy
        # Later timestamps → apply rule
        if prev_best_ask is None or best_ask <= prev_best_ask + 1:
        ...
        if len(sorted_asks) >= 2:
            ...
            if prev_best_ask is None or second_best_ask <= prev_best_ask + 3:
                orders.append(Order(product, second_best_ask, 5))

        # Store for next timestamp
        ts["prev_best_ask"] = best_ask
