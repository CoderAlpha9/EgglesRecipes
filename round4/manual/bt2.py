...
my_orders = {
    ...
    'AC_50_P':   {'action': 'BUY',  'vol': 6},
    'AC_50_C':   {'action': 'BUY',  'vol': 4},
    'AC_35_P':   {'action': 'SELL', 'vol': 0},
    'AC_40_P':   {'action': 'SELL', 'vol': 0},
    'AC_45_P':   {'action': 'BUY',  'vol': 0},
    'AC_60_C':   {'action': 'BUY',  'vol': 0},
    'AC_50_P_2': {'action': 'BUY',  'vol': 6},
    'AC_50_C_2': {'action': 'BUY',  'vol': 6},
    'AC_50_CO':  {'action': 'SELL', 'vol': 6},
    'AC_40_BP':  {'action': 'SELL', 'vol': 6},
    'AC_45_KO':  {'action': 'BUY',  'vol': 60}
}
...
def run_stress_test(n_rounds=10_000, sims_per_round=100):
    ...
    for chunk in range(0, n_rounds, chunk_size):
        ...
        for asset, order in my_orders.items():
            ...
            # Account for underlying multiplier correctly (1 for AC, 3000 for options)
            mult = 1 if asset == 'AC' else CONTRACT_MULTIPLIER
            total_pnl_per_sim += edge * vol * mult
    ...
    return {
        ...
        'raw_scores': round_scores # Added this specifically for the plotter
    }
...
if __name__ == "__main__":
    ...
    # ==============================================================================
    print("Generating Matplotlib Frequency Chart...")
    plt.figure(figsize=(12, 6))
    
    # Create the histogram using the raw_scores
    n, bins, patches = plt.hist(stats['raw_scores'], bins=100, edgecolor='black', alpha=0.7)
    
    # Color the bins: Red for losses (<= 0), Green for profits (> 0)
    ...
    # Add vertical lines for Mean and Breakeven
    plt.axvline(x=0, color='red', linestyle='--', linewidth=2, label='Breakeven ($0)')
    plt.axvline(x=stats['mean'], color='blue', linestyle='-.', linewidth=2, label=f"Mean ({int(stats['mean']):,})")
    
    # Add labels and title
    plt.title("Portfolio PnL Distribution (10,000 Intarian Rounds)", fontsize=14, pad=15)
    plt.xlabel("Total PnL (Intarian Credits)", fontsize=12)
    plt.ylabel("Frequency (Number of Rounds)", fontsize=12)
