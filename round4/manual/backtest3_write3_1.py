...
def precompute_vectorized_engine(n_rounds=5000, sims_per_round=100):
    ...
    print(f"Matrix Pre-computation complete in {time.time() - start_time:.2f} seconds.\n")
...
def eval_portfolio(portfolio, buy_scores, sell_scores, greeks, n_rounds):
    ...
    # Priority Tuple: 1. Max Loss Floor, 2. Delta to 0, 3. Average PnL
    fitness = (worst_case - penalty_vol, -abs(net_delta), avg_pnl)
    return fitness, worst_case, best_case, avg_pnl, net_delta, net_gamma

# ==============================================================================
# 4. SINGLE RUN OPTIMIZER
# ==============================================================================
def optimize_single_run(buy_scores, sell_scores, greeks, n_rounds, max_iterations=500):
    init_portfolio = {}
    while True:
        for asset in gui_order:
            if asset != 'AC':
...
def run_global_optimization(n_restarts=100):
    ...
    print(f"Initiating {n_restarts} Independent Optimization Runs...")
    global_best_fit = (-float('inf'), -float('inf'), -float('inf'))
    global_best_metrics = None
    
    for run in range(1, n_restarts + 1):
        fit, metrics = optimize_single_run(buy_scores, sell_scores, greeks, n_rounds, max_iterations=500)
        
        if fit > global_best_fit:
            global_best_fit = fit
            global_best_metrics = metrics
            print(f"[*] Run {run:03d} | NEW BEST! Max Loss: {int(metrics['worst']):,} | Avg: {int(metrics['avg']):,} | Delta: {metrics['delta']:.2f}")
        elif run % 10 == 0:
            print(f"[ ] Run {run:03d} | Peak found but did not beat global best.")
...
if __name__ == "__main__":
    np.random.seed()
    # Execute the 100-run global search
