...
# ==============================================================================
# 3. HIGH-VOLUME COMPOSITE EVALUATOR (WITH DELTA BAND)
# ==============================================================================
def eval_portfolio(portfolio, buy_scores, sell_scores, greeks, n_rounds):
    ...
    # --- The Soft Delta Band ---
    # Delta strictly between -2.0 and +2.0 incurs zero penalty.
    delta_penalty = 0 if abs(net_delta) <= 2.0 else abs(net_delta) * 5_000_000
    
    # --- Minimum Volume Constraint ---
    vol_penalty = max(0, 200 - total_option_vol) * 10_000_000
    
    # --- Composite Objective Function ---
    # Balances High Avg PnL, High Floor, and High Gamma into one master score.
    fitness = avg_pnl + (worst_case * 10.0) + (net_gamma * 100.0) - delta_penalty - vol_penalty
    
    return fitness, worst_case, best_case, avg_pnl, net_delta, net_gamma

# ==============================================================================
# 4. SIMULATED ANNEALING CYCLICAL OPTIMIZER
# ==============================================================================
def optimize_sa_batch(buy_scores, sell_scores, greeks, n_rounds, max_iterations=500):
    init_portfolio = {}
    
    # Generate Valid Random Start inside the Tolerance Band
    while True:
        for asset in gui_order:
            if asset != 'AC':
    ...
    current_fit, w, b, a, d, g = eval_portfolio(init_portfolio, buy_scores, sell_scores, greeks, n_rounds)
    ...
    best_overall_portfolio = portfolio.copy()
    best_overall_fit = current_fit
    best_metrics = (w, b, a, d, g)
    ...
    for i in range(max_iterations):
        ...
        # Scan all logical neighbors
        for asset in gui_order:
            if asset == 'AC': continue 
            ...
            for d_vol in [-1, 1]:
                if abs(portfolio[asset] + d_vol) <= max_v:
                    ...
                    # Apply Hedge to keep within the Delta Band
        ...
        if best_n is not None:
            if best_n_fit > current_fit:
                # Accept improvement
                portfolio = best_n
                current_fit = best_n_fit
            else:
                # Accept worse move conditionally to escape local minima
                acceptance_probability = np.exp((best_n_fit - current_fit) / temperature)
                if np.random.rand() < acceptance_probability:
                    portfolio = best_n
                    current_fit = best_n_fit
            
            # Always track the absolute global best seen during the entire walk
            if current_fit > best_overall_fit:
                best_overall_fit = current_fit
                best_overall_portfolio = portfolio.copy()
                best_metrics = best_n_metrics
                
        # Cool down the system
        temperature = max(1.0, temperature * cooling_rate)
            
    w, b, a, d, g = best_metrics
    return best_overall_fit, {
        'portfolio': best_overall_portfolio,
        'worst': w, 'best': b, 'avg': a, 
        'delta': d, 'gamma': g,
        'composite_score': best_overall_fit
    }
...
def run_global_optimization(n_restarts=100):
    ...
    print(f"Initiating {n_restarts} Independent SA Batches...")
    global_best_fit = -float('inf')
    global_best_metrics = None
    
    for run in range(1, n_restarts + 1):
        fit, metrics = optimize_sa_batch(buy_scores, sell_scores, greeks, n_rounds, max_iterations=200)
        
        if fit > global_best_fit:
            global_best_fit = fit
            global_best_metrics = metrics
            print(f"[*] Batch {run:03d} | NEW GLOBAL BEST! Max Loss: {int(metrics['worst']):,} | Avg: {int(metrics['avg']):,} | Net Delta: {metrics['delta']:.2f}")
...
def manage_ledger(current_metrics):
    ...
    curr_row['Composite_Score'] = current_metrics['composite_score']
    ...
    if os.path.exists(LEDGER_FILE):
        try:
            ...
            if not df_hist.empty:
                # Rank historically by the Composite Score
                best_idx = df_hist['Composite_Score'].idxmax()
    ...
    if historical_best:
        ...
        if current_metrics['composite_score'] > historical_best['Composite_Score']:
            print("\n>>> NEW ALL-TIME BEST RECORD ESTABLISHED (Composite Score Improved) <<<")
    else:
