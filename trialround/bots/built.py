...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            best_ask, best_ask_vol_raw = sorted_asks[0]
            best_ask_vol = abs(best_ask_vol_raw) # Safety: ensure positive representation for feature math
            ...
            if len(feats["spread"]) > window_len: 
                feats["spread"].pop(0); feats["imb"].pop(0)
                feats["mdiv"].pop(0); feats["vol"].pop(0)
            ...
            if product == "TOMATOES":
                ...
                my_bid = int(math.floor(ideal_bid))
                my_ask = int(math.ceil(ideal_ask))
                ...
                if my_bid >= my_ask: 
                    # If spread collapses, revert to pennying safely
                    my_bid = best_bid
                    my_ask = best_ask
            ...
            # ==========================================
                ...
                # Apply VPIN Toxicity Shield *Before* Clamping
                if vpin > 0.20:
