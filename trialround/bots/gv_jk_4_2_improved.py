...
class Trader:
    ...
    def compute_vwap(self, order_depth: OrderDepth) -> float:
        weighted_bids = [k * order_depth.buy_orders[k] for k in order_depth.buy_orders]
        weighted_sells = [k * -order_depth.sell_orders[k] for k in order_depth.sell_orders]
        num = sum(weighted_bids) + sum(weighted_sells)

        bid_vol = sum(order_depth.buy_orders.values())
        ask_vol = -sum(order_depth.sell_orders.values())
        total_vol = bid_vol + ask_vol

        return num / total_vol if total_vol > 0 else 0
    ...
    def run(self, state: TradingState):
        ...
        # --- State Management ---
        trader_state = {"TOMATOES_HISTORY": []} #, "EMERALDS_HISTORY": []}
        if state.traderData:
            try:
        ...
        for product in state.order_depths:
            ...
            sorted_ask = sorted(order_depth.sell_orders.keys())
            sorted_bid = sorted(order_depth.buy_orders.keys())
            
            best_ask = sorted_ask[0]
            best_bid = sorted_bid[-1]
            mid_price = (best_bid + best_ask) / 2.0
            vwap_price = self.compute_vwap(order_depth=state.order_depths[product])

            if best_ask < best_bid:
                s_vol = min(-order_depth.sell_orders[best_ask], limit - current_pos)
                orders.append(Order(product, best_ask, s_vol))

                b_vol = max(-order_depth.buy_orders[best_bid], -limit - current_pos)
                orders.append(Order(product, best_bid, b_vol))
            ...
            # ==========================================
                ...
                recent_sell_pressure = sum(t.quantity for t in market_trades if t.price <= best_bid) > 20
                recent_buy_pressure = sum(t.quantity for t in market_trades if t.price >= best_ask) > 20

                is_crashing = slope < -0.4 or (slope < -0.2 and recent_sell_pressure)
                is_rocketing = slope > 0.4 or (slope > -0.2 and recent_buy_pressure)
                
                # Extreme Inventory Danger (Stop-Loss Pivot)
                if current_pos > 40 and is_crashing:
                    dump_vol = -limit - current_pos
                    orders.append(Order(product, best_bid, dump_vol))
                    current_pos += dump_vol
                elif current_pos < -40 and is_rocketing:
